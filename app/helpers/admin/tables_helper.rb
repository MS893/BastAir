...
module Admin
  module TablesHelper
    ...
    def display_cell_content(record, column, table_name)
      ...
      when :boolean
        if value
          ...
          content_tag(:span, 'Non', class: 'badge bg-danger')
        end
    ...
    def header_text_for(table_name, column_name)
      ...
      when %w[avions immatriculation]
        'Immat'
      when %w[vols nb_atterro]
        'Nb att'
      else
    ...
    def display_association_name(_record, column, _table_name, value)
      return unless value
