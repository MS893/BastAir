...
class SignalementsController < ApplicationController
  ...
  before_action :authorize_instructor_or_admin!, only: %i[edit update destroy]
      ...
      if @signalement.save
        ...
        format.html do
          redirect_to signalements_path,
                      notice: "Le signalement sur l'avion #{@avion.immatriculation} a été enregistré avec succès."
        end
        ...
        format.html do
          redirect_to signalements_path(by_avion: @avion.id),
                      alert: "Le signalement n'a pas pu être créé : #{error_message}"
        end
