...
class ElearningController < ApplicationController
  ...
  def show
    ...
    return unless File.exist?(markdown_file_path)
  ...
  def document
    if @course.document.attached?
      ...
      send_data @course.document.download, filename: @course.document.filename.to_s,
                                           type: @course.document.content_type, disposition: 'inline'
    else
