  ...
  describe 'Authenticated user access' do
    context 'with a valid user' do
      ...
      it 'sends a confirmation email' do
        ...
        expect do
          post :create, params: { reservation: reservation_params }
        end.to have_enqueued_mail(UserMailer)
      end
      ...
      describe 'DELETE #destroy' do
        ...
        let!(:reservation_to_delete) do
          create(:reservation, user: valid_user, avion: avion, start_time: start_t, end_time: end_t)
        end
        ...
        it 'creates a penalty when cancellation is late (within 12h)' do
          ...
          end.to change(Penalite, :count).by(1)
        ...
        context 'when deleting an instruction flight' do
          ...
          let!(:instruction_reservation) do
            r = build(:reservation, user: valid_user, avion: avion, instruction: true, fi: instructor.name,
                                    start_time: start_t + 1.day, end_time: end_t + 1.day)
        ...
        it 'does not create a penalty when cancellation is early enough' do
          early_reservation = create(:reservation, user: valid_user, avion: avion, start_time: start_t + 48.hours,
                                                   end_time: end_t + 48.hours)

          expect do
            delete :destroy, params: { id: early_reservation.id, cancellation_reason: 'Vacances' }
          end.not_to change(Penalite, :count)
        end
        ...
        it "does not debit the user immediately (penalty status is 'En attente')" do
          ...
          expect do
            delete :destroy, params: { id: reservation_to_delete.id, cancellation_reason: 'Malade' }
          end.not_to(change { valid_user.reload.solde })
        end
      end
    end
