...
RSpec.describe ProgressionsController, type: :controller do
  ...
  describe 'POST #update_exam' do
    ...
    it 'updates student status to brevete' do
      post :update_exam, params: { eleve_id: student.id, user: { date_fin_formation: Time.zone.today } }
      student.reload
      ...
      expect(student.date_licence).to eq(Time.zone.today)
    end
  end
  ...
  describe 'POST #send_exam_email' do
    ...
    it 'sends email if exam validated' do
      student.update(date_fin_formation: Time.zone.today)
